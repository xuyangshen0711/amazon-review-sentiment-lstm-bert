"""
CS6120 Final Project - LSTM Pipeline (Original / Unweighted)
Author: Xuyang Shen
...
def load_amazon_data(file_path, max_reviews=100000):
    ...
    This function is identical to the BERT pipeline to ensure
...
# Per-class metrics
precision_per, recall_per, f1_per, support_per = precision_recall_fscore_support(test_targets, test_preds_binary, average=None)
print(f"\nPer-class metrics:")
print(f"  Negative - Precision: {precision_per[0]:.4f}, Recall: {recall_per[0]:.4f}, F1: {f1_per[0]:.4f}, Support: {support_per[0]}")
print(f"  Positive - Precision: {precision_per[1]:.4f}, Recall: {recall_per[1]:.4f}, F1: {f1_per[1]:.4f}, Support: {support_per[1]}")
...
metrics_lines = [
    ...
    f"Recall: {recall:.4f}",
    f"F1-Score: {f1:.4f}",
    f"Negative Precision: {precision_per[0]:.4f}",
    f"Negative Recall: {recall_per[0]:.4f}",
    f"Negative F1: {f1_per[0]:.4f}",
    f"Negative Support: {support_per[0]}",
    f"Positive Precision: {precision_per[1]:.4f}",
    f"Positive Recall: {recall_per[1]:.4f}",
    f"Positive F1: {f1_per[1]:.4f}",
    f"Positive Support: {support_per[1]}",
    "Confusion Matrix:",
    str(conf_matrix),
]
...
# --- CELL 9: Visualizations (aligned with BERT format) ---
...
# 3. Confusion Matrix Visualization (same format as BERT)
...
plt.title('LSTM (Unweighted) - Confusion Matrix on Test Set')
plt.tight_layout()
...
summary_lines = [
    ...
    f"  Neg Class Recall:  {recall_per[0]:.4f}",
]
